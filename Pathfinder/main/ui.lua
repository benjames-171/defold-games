...
function M.show(node)
	...
	gui.animate(node, "scale.y", 1, gui.EASING_INOUTQUAD, M.FADE)
end
...
function M.hide(node)
	...
	gui.animate(node, "scale.y", M.SCALE, gui.EASING_INOUTQUAD, M.FADE)
end

function M.init(node)
	gui.set_position(node, vmath.vector3(data.SCR_W/2, (data.SCR_H/2) - M.GUI_DROP, 0))
	gui.set_scale(node, vmath.vector3(1,M.SCALE,1))
	gui.set_enabled(node, false)
